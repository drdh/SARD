...
class LoggerRL:
    ...
    def __init__(self, init_stats_logger=True, use_c_reward=False, log_done_reason=True):
        self.use_c_reward = use_c_reward
        self.num_steps = 0
        self.num_episodes = 0
        self.sample_time = 0
        self.stats_names = ['episode_len', 'reward', 'episode_reward']
        self.log_done_reason = log_done_reason
        if self.log_done_reason:
            self.stats_names.extend(['done_isfinite', 'done_min_height', 'done_max_height',
                                     'done_angle', 'done_max_nsteps'])
        self.stats_nparray = []
    ...
    def step(self, env, reward, c_reward, c_info, info):
        ...
        if self.log_done_reason and info.get('done', False):
            for k, v in info.items():
                if k in self.stats_names:
                    self.stats_loggers[k].log(v)
