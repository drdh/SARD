...
class Joint:
    ...
    def sync_node(self):
        ...
        if 'range' in self.node.attrib:
            self.node.attrib['range'] = '-60 60'
        if self.type == 'hinge':
            if self.cfg['axis_vertical']:
                bone_offset = self.body.bone_offset
                # rotation90 = np.array([
                #     [0, -1, 0],
                #     [1, 0, 0],
                #     [0, 0, 1.],
                ...
                axis_vec = (rotation90 @ bone_offset.reshape(3,1)).reshape(3)
                if np.allclose(np.linalg.norm(axis_vec), 0):
...
class Geom:
    ...
    def update_start(self):
        if self.type == 'capsule':
            ...
            self.start = self.bone_start + vec * (self.ext_start / (np.linalg.norm(vec) + 1e-6))
...
class Body:
    ...
    def reindex(self):
        if self.parent is None:
            ...
            self.name = np.base_repr(ind, 36) + pname
    ...
    def set_params(self, params, pad_zeros=False, map_params=False):
        if map_params:
            # if self.parent is not None:
            #     params[:2] += self.robot.demap_params(self.parent.bone_offset[:2])
        ...
        if self.bone_offset is not None and 'offset' in self.param_specs:
            if self.param_specs['offset']['type'] in {'xz', 'xy'}:
                ...
                # if np.all(offset == 0.0):
                if np.allclose(np.linalg.norm(offset[:2]), 0):
                    offset[0] = 0.001 # 1e-8

                offset[2] = - np.linalg.norm(offset[:2]) * np.sin(np.pi/6)
